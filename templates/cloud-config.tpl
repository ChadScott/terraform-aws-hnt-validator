...
write_files:
  - path: /usr/local/bin/mount-validator-volume
    ...
    content: |
      ...
      volume_id="${replace(volume_id, "-", "")}"

      while [[ ! $(nvme list -o json | jq -r ".Devices[] | select(.SerialNumber==\"$${volume_id}\").DevicePath") ]]; do
      ...
      if [ ! -d /usr/local/mnt/validator ]; then
        mkdir -p /usr/local/mnt/validator
      fi

      if ! grep -qs "LABEL=validator" /etc/fstab; then
        echo "LABEL=validator /usr/local/mnt/validator ext4 defaults 0 2" >> /etc/fstab
      fi

      mount -a

      chown root:root /usr/local/mnt/validator
